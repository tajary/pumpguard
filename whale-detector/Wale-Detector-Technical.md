...
flowchart TD
    ...
    B --> C[Load Token Contract<br>(symbol, decimals, totalSupply)]
    C --> D[Call get_transfer_events()]
    D --> E[PolygonScan V2 API<br>getLogs → Transfer events]
    E --> F[Extract from/to addresses<br>→ Unique active holders]
    ...
    K --> L[Label: EOA vs Contract<br>+ Known names]
