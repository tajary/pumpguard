...
// Future: ML-powered detection
$mlEndpoint = 'http://ml-service:5000/predict';
$predictions = callMLService($mlEndpoint, $recentSwaps);
foreach ($predictions as $prediction) {
    if ($prediction['anomaly_score'] > 0.7) {
        // Create alert with ML confidence score
    }
}
```

**ML Integration Points:**
- `backend/analyze_swaps.php` - Line 45 (marked with comment)
- Add Python Flask microservice for ML models
- Train on historical pump/dump patterns
- Use features: volume, trader diversity, time patterns, price impact
